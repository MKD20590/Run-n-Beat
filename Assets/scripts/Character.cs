...
/*            if(hit.collider.tag=="ground")
        ...
        //movement
        if (!gm.load && move != Vector3.zero && !dashed && moved && !gm.died && !gm.win)
        {
        ...
        //jumping
        if(!gm.load && Input.GetKeyDown(KeyCode.Space) && jumped < 2 && !gm.died && !gm.win)
        {
            jumped++;
/*            if(!isGrounded && jumped==1)
            {
                jumped=2;
                body.ResetTrigger("jump1");
        ...
        //dashing
        if(!gm.load && !gm.died && !gm.win && !hurted && move != Vector3.zero && Input.GetKeyDown(KeyCode.LeftShift) || !gm.load && Input.GetKeyDown(KeyCode.RightShift) && !dashed && !hurted && !gm.died && !gm.win)
        {
            trail.emitting = true;
            moved = false;
            dashes.Play();
        ...
        //player rotation
        if(!gm.load && move != Vector3.zero && !gm.load && !gm.died && !gm.win)
        {
...
/*            if(main_coll.enabled)
    ...
    {
        //laser collision
        if (collision.gameObject.tag == "laser" && !hurted && !gm.win && !gm.died && !gm.load)
        {
            hp--;
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (isGrounded && collision.gameObject.tag == "ground" && !gm.win && !gm.died && !gm.load)
        {
            if (body.GetCurrentAnimatorStateInfo(0).IsName("onAir"))
                jumped = 0;
        }
        /*        if (isGrounded && !gm.win && !gm.died && !gm.load)
                {
                    jumped = 0;
                    //Debug.Log("land");
                    if (body.GetCurrentAnimatorStateInfo(0).IsName("land") || body.GetCurrentAnimatorStateInfo(0).IsName("idle") || body.GetCurrentAnimatorStateInfo(0).IsName("walk"))
                    {
                        jumped = 0;
                    }
                }*/
        ...
        //spike collision
        if (collision.gameObject.tag == "spike" && !hurted && !gm.win && !gm.died && !gm.load)
        {
            hp--;
            player_material.SetColor("_EmissionColor", Color.red * 2.5f);
            if(hp>0)
